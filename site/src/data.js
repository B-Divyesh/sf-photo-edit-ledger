...
export function assessRoute(sourceId, destinationId) {
  ...
  if (destinationId === 'immich-readonly') note = 'Read-only extraction can overwrite catalog changes. Keep the editor catalog authoritative and preserve every sidecar.';
  else if (fields.some((field) => field.key === 'adjustments' && field.state !== 'portable')) note = 'Render critical edited versions and keep the original sidecar; proprietary recipes are not translated.';
